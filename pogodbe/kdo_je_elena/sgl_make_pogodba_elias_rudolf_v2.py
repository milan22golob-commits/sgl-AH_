"""
sgl_make_pogodba_elias_rudolf_v2.py
==================================
Generira AVTORSKO POGODBO za Eliasa Rudolfa (v2) — predstava KDO JE ELENA?

Osnova: Nina Šorak MG (20. 8. 2026) kot trenutni gold standard + popravki iz
REVIEW-a treh pravnikov (DP / AP / ZP, 14. 8. 2026) k Elias v1_PREDLOG.

Elias-specifika (dvojna avtorska vloga v eni pogodbi, brez vmesnika):
1. Avtor IZVIRNEGA dramskega dela "Kdo je Elena?" (5. člen ZASP)
2. Režiser uprizoritve                          (5. člen ZASP)

Honorar: 2.300,00 EUR bruto SKUPAJ za obe avtorski vlogi
Datum pogodbe: 1. september 2026 (Milan po potrebi prilagodi)
Premiera: marec 2027, Šentjakobski oder
Izvod: 3 (1 izvajalec, 2 naročnik)

Ključne razlike v2 vs v1_PREDLOG:
- 3. člen: režija zdaj sklic na 5. člen ZASP (ne 80. — popravek AP-review)
- 6. člen: prenos MAP s SPECIFIČNIMI ZASP členi (23., 24., 26., 30., 32.a)
- 7. člen: trajanje repertoarja + izrecna REVERZIJA (ob umiku pravice nazaj)
- 8. člen: varovalka avtorja obdrži pravice na besedilu (nespremenjena)
- 11. člen: jamstvo z opombo o SORODNIH pravicah tretjih (igralci, glasba)
- 12.-16. člen: mehčanje delovno-pravnih indicij (razpored vaj sporazumno,
  brez "hišnega reda", obveščanje naročnika namesto inšpicienta takoj,
  sporazumno določena nadomestna rešitev pri odsotnosti)
- 5. člen: eksplicitno pojasnilo o bruto-znesku in izplačilu skladno s
  statusom (ostane hibridna izplačilna klavzula)

Manjkajoči podatki (izpolni Elias sam pred podpisom):
- naslov stalnega bivališča, davčna številka, davčni urad, status, TRR

Output (Windows lokalna pot — poteče iz Milanovega Google Drive Streama):
  Šentjakobsko gledališče\\Pogodbe in računi\\Kdo je Elena\\
  ŠGL_avtorska pogodba_KDO JE ELENA_Elias Rudolf_avtor-režiser_v2.docx

Standard: v7 (9-poglavna struktura + sgl_docx_format tipografija).
"""
...
OUT_DOCX = OUT_DIR / 'ŠGL_avtorska pogodba_KDO JE ELENA_Elias Rudolf_avtor-režiser_v2.docx'
...
def init_doc():
    ...
    tmp = OUT_DIR / '_tmp_elias_v2.docx'
...
def main():
    ...
    # Elias Rudolf (F.O.)
    add_para_runs(doc, [('g. Elias Rudolf', True)])
    add_para_runs(doc, [('naslov stalnega bivališča: ________________________________________', False)])
    add_para_runs(doc, [('davčna številka: ____________________', False)])
    add_para_runs(doc, [('davčni urad: ____________________', False)])
    add_para_runs(doc, [('davčni zavezanec za DDV: NE', False)])
    add_para_runs(doc, [('status: ___________________________', False)])
    add_para_runs(doc, [('TRR: ___________________________ (banka: __________________)', False)])
    ...
    # 3. člen — POPRAVEK: režija po 5. čl. (ne 80.)
    add_clen(doc)
    ...
    # 4. člen — MEHČANJE (koordinacija namesto vodenje; brez "prednostno redno zaposlene")
    add_clen(doc)
    add_para_runs(doc, [
    ...
    # 5. člen — 2.300 EUR bruto skupaj, hibridna izplačilna klavzula
    add_clen(doc)
    add_para_runs(doc, [
        ...
        ('2.300,00 EUR bruto', True),
        ('. Ker izvajalec ni zavezanec za DDV, se ta ne obračuna.', False),
    ])
    add_para_runs(doc, [
    ...
    add_para_runs(doc, [
        ('Naročnik izplača honorar na transakcijski račun izvajalca skladno z njegovim davčnim statusom — bodisi na podlagi računa, ki ga izvajalec izstavi, bodisi z obračunom akontacije dohodnine in obveznih prispevkov, ki ga pripravi naročnik kot izplačevalec, kadar izvajalec računa ne more izstaviti.', False),
    ])
    add_para_runs(doc, [
        ('Naročnik izplača honorar najkasneje trideseti (30.) dan po premieri uprizoritve.', False),
    ])

    # ============== IV. PRENOS MATERIALNIH AVTORSKIH PRAVIC ==============
    add_section(doc, 'PRENOS MATERIALNIH AVTORSKIH PRAVIC')

    # 6. člen — SPECIFIČNI ZASP členi (AP-review)
    add_clen(doc)
    add_para_runs(doc, [
    ...
    # 7. člen — trajanje + REVERZIJA (AP-review novost)
    add_clen(doc)
    add_para_runs(doc, [
    ...
    # 8. člen — VAROVALKA (Elias-specifika ostane)
    add_clen(doc)
    add_para_runs(doc, [
    ...
    # 11. člen — JAMSTVA + OPOMBA o sorodnih pravicah tretjih (AP-review)
    add_clen(doc)
    ...
    # 12. člen — MEHČANJE (razpored sporazumno, brez "hišnega reda", brez "tedenskega programa hiše")
    add_clen(doc)
    add_para_runs(doc, [
    ...
    # 13. člen — MEHČANJE (brez "hišnega reda"; splošno gospodarno ravnanje)
    add_clen(doc)
    add_para_runs(doc, [
    ...
    # 14. člen — promocija v OMEJENEM obsegu (soft)
    add_clen(doc)
    add_para_runs(doc, [
    ...
    # 15. člen — OBJEKTIVNI RAZLOGI: sporazumna nadomestna rešitev (kot Nina MG)
    add_clen(doc)
    add_para_runs(doc, [
    ...
    # 16. člen — OBVEŠČANJE naročnika (ne "inšpicient takoj")
    add_clen(doc)
    add_para_runs(doc, [
    ...
    # 18. člen — odstop izvajalca + pravice na besedilu ostanejo
    add_clen(doc)
    add_para_runs(doc, [
